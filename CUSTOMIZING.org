...
Dirvish needs no additional configuration to work with TRAMP.  You have access
to most of the features that work on localhost inside of a TRAMP connection
except those expensive ones (git-msg, image preview, etc).  The other caveat is
that if want use the =dirvish-yank= extension for file transfer between different
host, you'll need to have a proper ssh config such as ssh-agent, because the
~rsync~ command is always run locally as it requires working SSH authentication
which bypasses the password entering to work.

You can improve the speed and responsiveness of TRAMP by tweaking its options.
See [[https://www.gnu.org/software/tramp/#Frequently-Asked-Questions][TRAMP-FAQ]] and example config below.
...
#+begin_src emacs-lisp
;; Some tips to speed Dirvish sessions over TRAMP
(use-package tramp
  :config
