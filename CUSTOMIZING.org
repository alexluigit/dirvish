...
ignored in Dirvish.  For those want to tweak the appearance of mode-line, just
...
*** Sample 2: preview PDF files with generated thumbnail
...
Some preview dispatchers generate cache images to improve the preview
experience.  Everytime you enter a directory, Dirvish scans the the content of
that directory and determines the fileset within the directory that requires
cache image for preview, the corresponding cache images for the fileset are
generated later when Emacs is idle.

You can tweak the behavior of auto caching or turn off this feature completely
by customizing the ~dirvish-auto-cache-threshold~ option.
